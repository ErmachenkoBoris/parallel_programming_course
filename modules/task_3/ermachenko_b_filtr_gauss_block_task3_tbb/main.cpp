...
double** ParallelFilterGaussTBB(double** arrImage,
    ...
    int eff = 0;
        ...
        int column = 0;
        int count = 0;
        parallel_for(blocked_range2d<int>(0, height, lengthY[0], 0, width, lengthX[0]), [&](const blocked_range2d<int, int>& r)
        {
            int eff = 0;
            printf("1 \n");
...
int main() {
    double tStart, tEnd, tRezultParallel, tRezultLinier;
    int height = 5000;
    int width = 5000;
    int nthreads = 0;
    int tid = 0;
    ...
    tick_count end = tbb::tick_count::now();
